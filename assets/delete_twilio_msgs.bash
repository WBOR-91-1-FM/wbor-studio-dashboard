auth_details=`jq -r '[.twilio_account_sid, .twilio_auth_token] | join(" ")' api_keys.json`
...
for ((i=0; i<$deletion_count; i++)) do
	# TODO: how to read these four with just one `jq` command?
	message_info=`jq -r ".messages[$i]" <<< $msgs_json`
	read uri from <<< `jq -r '[.uri, .from] | join(" ")' <<< $message_info`
	...
	echo -e "Deleted message: '$body' (from '$from', on '$date_sent')\n---"
done
